...
def main():
    ...
    # Read inputs before entering atomic write block
    source_manifest_path = os.path.join(args.dataset, "manifest.csv")
    is_synthetic = not os.path.exists(source_manifest_path)

    if is_synthetic:
        if not args.allow_synthetic_placeholder:
            raise FileNotFoundError(
                f"Source classic dataset manifest not found at: {source_manifest_path}. "
                "Please generate the classic dataset first, or pass --allow-synthetic-placeholder."
            )

    def write_bank(temp_dir):
            ...
            for fam in families:
                for var_name, scales in VARIANTS.items():
                    ...
                    rows.append({
                        ...
                        "family": fam,
                        "variant": var_name,
                        "pid_path": os.path.relpath(pid_path, temp_dir),
                        "score": 0.42 if var_name == "conservative" else (0.48 if var_name == "base" else 0.61),
                    })
            manifest = pd.DataFrame(rows)
            manifest.to_csv(os.path.join(temp_dir, "pid_bank_manifest.csv"), index=False)
            print(f"Synthetic outer force PID bank written to {temp_dir}")
            return
            ...
            if args.workers == 1:
                for index, source_row in enumerate(real_rows, start=1):
                    scenario_rows, pid_writes = _evaluate_source_scenario(
                        args.dataset, temp_dir, source_row
                    )
                ...
                with ProcessPoolExecutor(max_workers=max_workers) as executor:
                    future_to_index = {
                        executor.submit(
                            _evaluate_source_scenario, args.dataset, str(temp_dir), source_row
                        ): index
        ...
        for _, srow in src_manifest.iterrows():
            ...
            if has_scenario_path:
                ...
                try:
                    ...
                    row_seed = int(srow.get("seed", local_seed))
                except Exception:
            ...
            for var_name, scales in VARIANTS.items():
                ...
                valid = 0
                total = 0
                variant_result_dir = os.path.join(temp_dir, "results", pid_id)
                ...
                if has_scenario_path:
                    ...
                    for idx, (gname, traj) in enumerate(geometries):
                        for prof in profiles:
                            total += 1
                            try:
                                tel, met = _run_outer_variant(fam, pid_config, traj, prof, row_seed + idx, temp_dir)
                                sc = pid_candidate_score(met, tel, fam)
                                ok, _ = passes_hard_filters(met, fam)
                                if ok:
                                    valid += 1
                ...
                rows.append({
                    ...
                    "variant": var_name,
                    "pid_path": os.path.relpath(pid_path, temp_dir),
                    "result_dir": os.path.relpath(variant_result_dir, temp_dir),
        ...
        if bad_scens:
            ...
            print("ERROR: One or more scenarios have zero safe outer PID candidates after all variants and hard filters.", file=sys.stderr)
            print(json.dumps(failure, indent=2, default=float), file=sys.stderr)
            raise ValueError(f"Safe candidates check failed. {len(bad_scens)} scenario(s) have no safe candidates.")

        # Success path: perform deferred pid yaml writes
        for pid_path, pid_dict in pid_writes_to_do:
            with open(pid_path, "w") as f:
                yaml.safe_dump(pid_dict, f, sort_keys=False)

        pd.DataFrame(rows).to_csv(os.path.join(temp_dir, "pid_bank_manifest.csv"), index=False)

    # Call atomic write directory helper
    atomic_write_directory(args.out, write_bank, args.overwrite)
    print(f"Outer force PID bank written to {args.out}")
