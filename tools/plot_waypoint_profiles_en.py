...
def main():
    ...
    for ax in axs[:, 0]:
        ax.grid(True)
        # Fase Aceleración
        ax.axvspan(0, t_acc_trap, color=colors_phase['acc'], alpha=0.5, label='Acceleration' if ax == axs[0,0] else "")
        # Fase Crucero
        ax.axvspan(t_acc_trap, t_acc_trap + t_const_trap, color=colors_phase['const'], alpha=0.5, label='Cruise' if ax == axs[0,0] else "")
        # Fase Frenado
        ax.axvspan(t_acc_trap + t_const_trap, t_total_trap, color=colors_phase['dec'], alpha=0.5, label='Deceleration' if ax == axs[0,0] else "")
    ...
    axs[0, 1].set_title(f"Triangular Profile ($L={L_tri}$ m)")
    ...
    # Guardar
    out_path = "TFG_Memoria/Figuras_en/diagramas/FIG-010.pdf"
    out_png = "TFG_Memoria/Figuras_en/diagramas/FIG-010.png"
