...
\begin{tikzpicture}[
  ...
  % ==========================================
  % BODY B REFERENCE FRAME (FRD)
  % ==========================================
  % x_B points forward (up)
  \draw[axisB, color=cyan!80!black] (CM) -- (0, 1.3) node[above, font=\sffamily\bfseries] {$x_B$ (Forward)};
  % y_B points right (right)
  \draw[axisB, color=magenta!80!black] (CM) -- (1.3, 0) node[right, font=\sffamily\bfseries] {$y_B$ (Right)};
  % z_B points down (into the page) — callout to the left
  ...
  \draw[violet!80!black, line width=1.2pt] ($(zSym)+(-0.08,-0.08)$) -- ($(zSym)+(0.08,0.08)$);
  \node[below=0.12cm of zSym, font=\sffamily\bfseries, color=violet!80!black] {$z_B$ (Down)};

  % ==========================================
  % QUADCOPTER ARMS (X Configuration)
  % ==========================================
  \draw[arm] (R1) -- (R2);
  \draw[arm] (R0) -- (R3);
  ...
  % ==========================================
  % ROTORS, PROPELLERS AND SPIN DIRECTIONS
  % ==========================================
  ...
  \node[draw=gray!30, fill=gray!3, rounded corners=2pt, below=3.0cm of CM, inner sep=5pt, minimum width=2.4cm, font=\sffamily\scriptsize] (leyenda) {
    \textbf{Sign of reaction torque $s_i$ (in yaw):} \\
    \textcolor{red!70!black}{$\bm{\curvearrowright}$} Clockwise (CW), $s_i = -1$\\
    \textcolor{blue!70!black}{$\bm{\curvearrowleft}$} Counter-clockwise (CCW), $s_i = +1$
  };
