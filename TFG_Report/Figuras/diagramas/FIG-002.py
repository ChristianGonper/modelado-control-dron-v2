...
def main():
    ...
    ax.text(len_W + 0.15, 0.0, 0.0, r"$x_W$ (East)", color='#15803d', fontsize=8.5, ha='left', va='center', fontweight='bold')
    
    # Eje y_W (Norte): azul
    ax.quiver(OW[0], OW[1], OW[2], 0, len_W, 0, color='#1d4ed8', arrow_length_ratio=0.12, lw=1.8, pivot='tail', zorder=3)
    ax.text(0.0, len_W + 0.15, 0.0, r"$y_W$ (North)", color='#1d4ed8', fontsize=8.5, ha='left', va='center', fontweight='bold')
    
    # Eje z_W (Arriba): rojo
    ax.quiver(OW[0], OW[1], OW[2], 0, 0, len_W, color='#b91c1c', arrow_length_ratio=0.12, lw=1.8, pivot='tail', zorder=3)
    ax.text(0.0, 0.0, len_W + 0.15, r"$z_W$ (Up)", color='#b91c1c', fontsize=8.5, ha='center', va='bottom', fontweight='bold')
    ...
    ax.text(OW[0] + 0.2, OW[1] - 1.2, OW[2], r"World reference" "\n" r"frame $\mathcal{W}$ (ENU)", color='black', fontsize=8.5, fontstyle='italic', ha='center')
    ...
    mid = (OW + OB) / 2.0
    ax.text(mid[0] - 0.1, mid[1] + 0.15, mid[2] + 0.1, r"$\mathbf{r}_{WB}$ (Position)", color='#4b5563', fontsize=8, ha='right', va='center')
    ...
    ax.text(OB[0] + (len_B + 0.12)*uxB[0], OB[1] + (len_B + 0.12)*uxB[1], OB[2] + (len_B + 0.12)*uxB[2], 
            r"$x_B$ (Forward)", color='#0891b2', fontsize=8.5, ha='left', va='center', fontweight='bold')
    ...
    ax.text(OB[0] + (len_B + 0.12)*uyB[0], OB[1] + (len_B + 0.12)*uyB[1], OB[2] + (len_B + 0.12)*uyB[2], 
            r"$y_B$ (Right)", color='#db2777', fontsize=8.5, ha='left', va='center', fontweight='bold')
    ...
    ax.text(OB[0] + (len_F + 0.1)*u_thrust[0], OB[1] + (len_F + 0.1)*u_thrust[1], OB[2] + (len_F + 0.1)*u_thrust[2], 
            r"$\mathbf{T}$" "\n" r"(Thrust)", color='#d97706', fontsize=9.5, fontweight='bold', ha='center', va='bottom', zorder=10)
    ...
    # Guardar
    out_pdf = "TFG_Report/Figuras/diagramas/FIG-002.pdf"
    out_png = "TFG_Report/Figuras/diagramas/FIG-002.png"
