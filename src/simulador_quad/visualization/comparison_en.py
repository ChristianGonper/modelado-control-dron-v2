...
ENGLISH_CONTROLLER_LABELS = {
    "classic_family_pid": "Family PID",
    "classic_pid_hold": "PID Hold",
    "classic_pid_circle": "PID Circle",
    "classic_pid_lissajous": "PID Lissajous",
    "classic_pid_waypoint": "PID Waypoint",
    "classic_pid_representative": "Rep. PD",
    "outer_force_oracle": "Force Oracle",
    "neural_outer_force_mlp": "MLP Force",
    "neural_outer_force_gru": "GRU Force",
    "neural_outer_force_lstm": "LSTM Force",
    "neural_position_mlp": "MLP Position",
    "neural_position_gru": "GRU Position",
    "neural_position_lstm": "LSTM Position",
}

def get_controller_style_en(controller_name: str) -> dict:
    style = get_controller_style(controller_name).copy()
    if controller_name in ENGLISH_CONTROLLER_LABELS:
        style["label"] = ENGLISH_CONTROLLER_LABELS[controller_name]
    elif controller_name.startswith("classic_pid_"):
        family = controller_name.replace("classic_pid_", "")
        style["label"] = f"PID {family.capitalize()}"
    elif controller_name.startswith("classic_transfer_"):
        fam = controller_name.replace("classic_transfer_", "")
        style["label"] = f"PID Transf. ({fam.capitalize()})"
    return style
...
def _prepare_comparison_dataframe(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    ...
    if ambiguous_mask.any():
        ...
        warnings.append(
            f"Omitted {dropped} rows with classic_family_pid: the actual PID cannot be safely inferred."
        )
...
def _mission_success_series(frame: pd.DataFrame, warnings: list[str] | None = None) -> pd.Series:
    ...
    if "success" in frame.columns:
        if warnings is not None:
            fallback_msg = (
                "Column mission_success missing; using success as a proxy for mission success."
            )
...
def _controller_labels(controllers: Sequence[str]) -> list[str]:
    ...
    return [short.get(str(ctrl), get_controller_style_en(str(ctrl))["label"]) for ctrl in controllers]
...
def _plot_xy_panel(ax, series: tuple[str, np.ndarray, np.ndarray, np.ndarray], *, title: str) -> None:
    ...
    ax.plot(position_W_m[:, 0], position_W_m[:, 1], color=COLORS["real"], linewidth=1.2, label="Tracking")
...
def _grouped_rmse_bars(
    ...
    *,
    ...
    for idx, ctrl in enumerate(controllers):
        ...
        if means:
            style = get_controller_style_en(ctrl)
            ax.bar(
                valid_x,
                means,
                width=width * 0.9,
...
def _plot_res_id_rmse_family(df: pd.DataFrame, output_dir: Path, formats: list[str] | None) -> list[str]:
    ...
    ax.set_xlabel("Family Evaluated in Test")
...
def _plot_res_ood_rmse_family(df: pd.DataFrame, output_dir: Path, formats: list[str] | None) -> list[str]:
    ...
    _grouped_rmse_bars(ax, frame, category_col="family", categories=families, controllers=controllers)
    ax.set_xlabel("Family Out of Distribution")
    ax.set_ylim(bottom=0)
    _legend_outside(ax, ncol=2)
...
def _plot_res_pid_transfer_matrix(pid_df: pd.DataFrame, output_dir: Path, formats: list[str] | None) -> list[str]:
    ...
    fig.colorbar(image, ax=ax, label="Mean RMSE [m]", fraction=0.046, pad=0.04)
    ...
    ax.set_xlabel("Evaluated Family")
    ax.set_ylabel("PD Tuned For")
...
def _plot_res_ood_termination_summary(df: pd.DataFrame, output_dir: Path, formats: list[str] | None) -> list[str]:
    ...
    classes = ["Completed", "Time", "Attitude", "Saturation", "Other"]
    controllers = _memory_controller_list(frame, "ood")
    counts = (
    ...
    colors = {
        "Completed": COLORS["gru"],
        "Time": "#7A869A",
        "Attitude": COLORS["failure"],
        "Saturation": COLORS["mlp"],
    ...
    ax.set_ylabel("Number of OOD Scenarios")
    ax.set_xlabel("Controller")
    ax.legend(
        loc="upper center",
...
def _plot_res_protections_ood(df: pd.DataFrame, output_dir: Path, formats: list[str] | None) -> list[str]:
    frame = df[(df["split"] == "ood") & df["controller"].isin(MEMORY_PRIMARY_CONTROLLERS)].copy()
    metrics = [
        ("degradation_percentage", "Mixer Degradation"),
        ("force_norm_clip_percentage", "Force Clipping"),
        ("force_tilt_clip_percentage", "Tilt Clipping"),
        ("saturation_percentage", "Actuator Saturation"),
    ]
    ...
    ax.set_xlabel("Mean Percentage in OOD [%]")
...
def _plot_res_trajectory_lemniscate(
    ...
    *,
    ...
    if not series:
        return [], "missing or incomplete telemetry"
    label_colors = {"MLP": COLORS["mlp"], "LSTM": COLORS["lstm"], "GRU": COLORS["gru"]}
    fig, axes = plt.subplots(2, 1, figsize=(6.2, 5.0))
    ref = series[0][3]
    axes[0].plot(ref[:, 0], ref[:, 1], "--", color=COLORS["reference"], linewidth=1.1, label="Reference")
...
def _plot_atlas_trajectories(output_dir: Path, formats: list[str] | None) -> tuple[list[str], list[tuple[str, str]]]:
    ...
    for figure_id, items in (("atlas_trayectorias_id", id_items), ("atlas_trayectorias_ood", ood_items)):
        ...
        if not loaded:
            skipped.append((figure_id, "missing telemetries"))
            continue
    ...
    if helix is None:
        skipped.append(("atlas_trayectoria_helix_3d", "missing telemetry"))
    else:
        _, _, position_W_m, reference_W_m = helix
        fig = plt.figure(figsize=(5.4, 4.5))
        ax = fig.add_subplot(111, projection="3d")
        ax.plot(reference_W_m[:, 0], reference_W_m[:, 1], reference_W_m[:, 2], "--", color=COLORS["reference"], label="Reference", linewidth=1.0)
        ax.plot(position_W_m[:, 0], position_W_m[:, 1], position_W_m[:, 2], color=COLORS["real"], label="Tracking", linewidth=1.2)
        ax.set_xlabel("East [m]")
        ax.set_ylabel("North [m]")
...
def plot_comparison(
    ...
    """
    Generate comparison plots C1 to C7 from aggregated campaign runs (English version).
    """
    ...
    skipped: dict[str, str] = {figure_id: "insufficient data in the CSV" for figure_id in ALL_FIGURE_IDS}
    ...
    if df.empty:
        raise ValueError(
            "Comparison CSV contains no usable rows after omitting ambiguous controllers."
        )
