...
def _plot_trajectory_xy(
    ...
    ax.plot(reference_W_m[:, 0], reference_W_m[:, 1], "--", color=COLORS["reference"], label="Referencia")
    ax.plot(position_W_m[:, 0], position_W_m[:, 1], color=COLORS["real"], label="Estado")

    # Start and end markers
    ax.scatter(position_W_m[0, 0], position_W_m[0, 1], color=COLORS["start"], s=45, label="Inicio", zorder=5)
    ax.scatter(position_W_m[-1, 0], position_W_m[-1, 1], color=COLORS["failure"], s=45, label="Fin", zorder=5)
...
def _plot_trajectory_3d_static(
    ...
    ax.plot(reference_W_m[:, 0], reference_W_m[:, 1], reference_W_m[:, 2], "--", color=COLORS["reference"], label="Referencia")
    ax.plot(position_W_m[:, 0], position_W_m[:, 1], position_W_m[:, 2], color=COLORS["real"], label="Estado")

    ax.scatter(position_W_m[0, 0], position_W_m[0, 1], position_W_m[0, 2], color=COLORS["start"], s=30, label="Inicio", zorder=5)
    ax.scatter(position_W_m[-1, 0], position_W_m[-1, 1], position_W_m[-1, 2], color=COLORS["failure"], s=30, label="Fin", zorder=5)

    if profile != "report":
        ax.set_title("Trayectoria 3D Estática")

    ax.set_xlabel("X_W Este [m]")
    ax.set_ylabel("Y_W Norte [m]")
    ax.set_zlabel("Z_W Arriba [m]")
    ax.legend(loc="best")
...
def _plot_tracking_error(
    ...
    fig, ax = plt.subplots()
    ax.plot(time_s, position_error_m, color=COLORS["failure"], label="Error L2")
...
def _plot_attitude_time(
    ...
    if profile != "report":
        fig.suptitle("Actitud del Vehículo (ENU/FRD)")
...
def _plot_angular_velocity_time(
    ...
    if profile != "report":
        fig.suptitle("Velocidad Angular Absoluta del Vehículo (Cuerpo)")
...
def _plot_control_effort(
    ...
    # Collective Thrust
    axes[0].plot(time_s, collective_thrust_N, color=COLORS["real"], label="Empuje")
    axes[0].set_ylabel("Empuje T [N]")
    ...
    if profile != "report":
        fig.suptitle("Esfuerzo de Control: Empuje Colectivo y Momentos")
    ...
    if saturation_flags is not None:
        ...
        for i, sat in enumerate(any_saturated):
            ...
            if sat and not in_sat:
                ...
                in_sat = False
                ax.axvspan(start_t, t, color=COLORS["failure"], alpha=0.15, label="Saturación" if "Saturación" not in ax.get_legend_handles_labels()[1] else "")
        if in_sat:  # Close final interval
            ax.axvspan(start_t, time_s[-1], color=COLORS["failure"], alpha=0.15, label="Saturación" if "Saturación" not in ax.get_legend_handles_labels()[1] else "")
...
def _plot_neural_outer_force(
    ...
    for idx, ax in enumerate(axes):
        ax.plot(time_s, desired_force_W_N[:, idx], "--", color=COLORS["reference"], label="Solicitado")
        if desired_force_clipped_W_N is not None:
            ax.plot(time_s, desired_force_clipped_W_N[:, idx], color=COLORS["real"], label="Aplicado (Clipped)")
        ax.set_ylabel(labels[idx])
        ax.legend(loc="best")

    axes[-1].set_xlabel("Tiempo [s]")
    if profile != "report":
        fig.suptitle("Fuerza Externa Neuronal (Solicitada vs Acotada)")
...
def _plot_perturbation_response(
    ...
    axes[0].plot(time_s, wind_W_m_s[:, 0], label="Wx (Este)", color=COLORS["real"])
    axes[0].plot(time_s, wind_W_m_s[:, 1], label="Wy (Norte)", color=COLORS["mlp"])
    axes[0].plot(time_s, wind_W_m_s[:, 2], label="Wz (Arriba)", color=COLORS["gru"])
    axes[0].set_ylabel("Viento W_W [m/s]")
    axes[0].legend(loc="best")

    # L2 Error response
    axes[1].plot(time_s, position_error_m, color=COLORS["failure"], label="Error L2")
    axes[1].set_ylabel("Error Posición [m]")
    axes[1].set_xlabel("Tiempo [s]")
    axes[1].legend(loc="best")

    if profile != "report":
        fig.suptitle("Respuesta del Dron a Perturbaciones de Viento")
...
def plot_telemetry(
    ...
    Args:
        ...
        profile: The visual style profile ('diagnostic' or 'report').
        formats: File formats to export, e.g. ['png', 'pdf'].
    ...
    with use_style(profile):
        ...
        # Wind perturbation
        # In telemetry: {"perturbation": {"wind_W_m_s": [wx, wy, wz]}}
        ...
        # desired_force_W_N is N x 3 (or containing nan if not set)
        if desired_force_W_N is not None and not np.all(np.isnan(desired_force_W_N)):
            # Replace nan with zeros if partially nan
