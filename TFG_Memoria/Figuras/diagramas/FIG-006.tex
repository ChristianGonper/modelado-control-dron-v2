...
\begin{tikzpicture}[
  ...
  \node[anchor=north west, font=\sffamily\bfseries, color=green!60!black] at ($(classic_group.south west) + (0.1,-0.15)$) {Módulo Clásico e Inferencia};
  ...
  % 1. Realimentación de posición a entradas (sale por la parte superior izquierda de planta)
  \draw[dashline] ([xshift=-0.4cm]planta.north) -- ++(0, 2.8) node[pos=0.7, right, align=right, font=\sffamily\scriptsize] {Posición $\bm{x}$\\Velocidad $\dot{\bm{x}}$} -- (entradas.west);

  % 2. Realimentación de actitud a lazo_interno (sale por la parte superior derecha de planta, sin flecha intermedia)
