...
\begin{tikzpicture}[
  ...
  \node[decision, below=0.8cm of diagnostico] (dec_base) {¿Supera umbral\\de RMSE?};
  ...
  \draw[line] (dec_base) -- node[right, font=\sffamily\bfseries\color{red!70!black}] {NO (Requiere sintonía)} (global);

  % ==========================================
  % FASE 3: FILTROS DUROS
  % ==========================================
  \node[box, below=of global] (filtros) {
    \textbf{Fase 3: Filtros Duros}\\
    Evaluar en bucle cerrado
  };
  \draw[line] (global) -- (filtros);

  % Criterios de filtro
  \node[decision, below=0.8cm of filtros] (dec_filtros) {¿Estable y seguro?};
  ...
  \node[box, below=of  supervivientes] (local) {
    ...
    para minimizar RMSE localmente
  };
  ...
  \node[box, below=of local] (seleccion) {
    ...
    1. Minimizar costo RMSE pos/vel\\
    2. Penalizar esfuerzo ($\int \|\bm{\tau}_B\|^2 dt$)\\
  ...
  \node[box, fill=gray!10, below=1.0cm of  seleccion, minimum width=4.5cm] (yaml) {
    ...
    Ganancias ganadoras congeladas en\\
    \texttt{data/classic\_dataset/v1/pids/}
  };
  \draw[line] (seleccion) -- (yaml);

  % Conexión directa desde Fase 1 (SÍ supera el umbral)
  \draw[line] (dec_base.east) -- node[above, xshift=0.3cm, font=\sffamily\bfseries\color{green!60!black}] {SÍ (Suficiente)} ++(2.8,0) |- (yaml.east);
