...
ingeniería del código. Pretende aislar los puntos que sostienen la
reproducibilidad del trabajo, el contrato de simulación,
...
\end{itemize}
Los scripts de \path{tools/} gestionan la generación de datasets, series de PD
...
cambiar el entorno físico. La ejecución puede sustituir el lazo externo por una
red, fijar expertos PD o ejecutar una serie OOD, pero el integrador, el
mixer, los actuadores, las perturbaciones y las métricas siguen siendo comunes.

\subsubsection*{Bucle de simulación multirate}
...
control con bloqueo de orden cero, se aplica el sistema de actuadores, se
...
El controlador neuronal no sustituye todo el piloto automático.
La red estima la fuerza deseada del lazo externo en el
...
Esta decisión facilita una interfaz común entre MLP, GRU y LSTM y evita
...
causa de finalización, saturación, degradación del mixer y metadatos de
procedencia. Esta separación permite reconstruir figuras de diagnóstico sin
